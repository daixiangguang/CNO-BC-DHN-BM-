...
for i = 1:maxgen
    [tx,ui]=DiscreteHopfieldParallel(alpha,beta,u,n,p,sizepop,d,initial_x,I1,I2,500);
...
[m1,n1]=size(x);
one1=single(gpuArray(eye(m1,m1)));
one2=single(gpuArray(eye(n1,n1)));
const1=0.5*d+beta*(I1-one1);
const2=alpha*(I2-one2);
const3=repmat(alpha*(-2+p)+beta*(-2*u+n),n,p*sizepop);

for t=1:iter
    %0.5*t[pos] + ALPHA * (s1_i - x_pos - 2 + P) + BETA * (s2_k - x_pos - 2 * U + N);
%     dedx=0.5*d*x+alpha*(x*I2-2+p-x*one2)+beta*(I1*x-2*u+n-one1*x);
