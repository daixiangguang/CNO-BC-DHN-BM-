...
#define POP 1 //种群数量 
#define SIZE N*P*POP 
#define ALPHA 200 //惩罚参数1
#define BETA  200//惩罚参数2
...
#define PSO_THREAD_NUM POP //最大线程数，如果不能除尽POP，设置成POP
#define PSO_BLOCKS SIZE/PSO_THREAD_NUM 

#define RD_THREAD_NUM POP
        ...
        if(it>=10)
{
        ...
        if (pb<curand_uniform(&localState))
             x[pos] = -1.0;
        else
            x[pos] = 1.0;
}
...
__global__ void stop(float *y, int *is_stop, int it,int STOPNUM)
{
	*is_stop = 0;
	if (y[it] == y[it - STOPNUM])
		*is_stop = 1;
}
int Hopfield_syn_cuda(float *fea,float *y,float *gbestx,float *initialx, int STOPNUM)
{
	...
	cudaMemcpy(dd, fea, sizeof(float) * N*N, cudaMemcpyHostToDevice);
	...
	cudaMemcpy(y, yd, sizeof(float)*gen, cudaMemcpyDeviceToHost);
