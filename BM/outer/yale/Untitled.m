...
//组内同步更新 
__global__ static void parallel_updated_GPU(float *gradients,float *obj, float *lbest, float *lbestx, float*initialx, float *x, float *d, float *s1, float *s2, float *t, int *index, int gen, curandState *globalState)
{
...
float T0=1000;
		...
		for (loop = 0; loop < N; loop++)
		{
			...
			gradients[pos] = 0.5*t[pos] + ALPHA * (s1_i - x_pos - 2 + P) + BETA * (s2_k - x_pos - 2 * U + N);
