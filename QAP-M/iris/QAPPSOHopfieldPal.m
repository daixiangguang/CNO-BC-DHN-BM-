...
for i = 1:maxgen
    ...
    constraint1=(tx*I2-2+p).*(tx*I2-2+p);
    constraint1=constraint1(:,1:p:end);
    constraint1=sum(constraint1,1);
    constraint2=(I1*tx-2*u+n).*(I1*tx-2*u+n);
    constraint2=constraint2(1,:);
    constraint2=reshape(constraint2,p,[]);
    ...
    obj=diag((tx'*(d-diag(diag(d)))*tx));
    obj=reshape(obj,p,[]);
    obj=(sum(obj,1)+constraint)*0.5;
    %局部最优
    for k=1:sizepop
        if obj(k)<pbest(k)
