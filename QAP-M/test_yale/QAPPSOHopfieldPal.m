...
C1=((2));
C2=((2));
y=zeros(maxgen,1);y=((y));
ui=zeros(n,sizepop*p);ui=(ui);
...
I2=((I2));
stopNum=1000;
m=zeros(n,sizepop*p);m=((m));


one=((eye(n,n)));
const1=0.5*d+beta*(I1-one);

one=((eye(p*sizepop,p*sizepop)));
const2=alpha*(I2-one);
...
x=cell(sizepop,1);
iter=50;
initialxx=zeros(n,sizepop*p);
for i = 1:maxgen
%     [tx]=DiscreteHopfieldParallel11(m,initial_x,const1,const2,const3,ITER);
   parfor j=1:sizepop
    [x{j},obj] = BM_HopfieldParallel1(initial_x{j},d,p,beta,alpha,iter,sizepop,n);
    end
    ...
    if pbest(best_id)<zbest && flag(best_id)==0
    zbest_x = pbest_x(:,(best_id-1)*p+1:p*best_id);
    zbest=pbest(best_id);
    end
    
%     for k=1:pop
%         initial_xx(:,(k-1)*p+1:p*k)=initial_x{k};
%     end
     for k=1:pop
    rd1=((rand(n,p)));
    rd3=((rand(n,p)));
    initial_v(:,(k-1)*p+1:p*k)=initial_v(:,(k-1)*p+1:p*k)+C1*rd1.*((pbest_x(:,(k-1)*p+1:p*k) + 1) / 2.0 - (initial_x{k} + 1) / 2.0)+C2*rd1.*(repmat(zbest_x+1,1,1) / 2.0 - (initial_x{k} + 1) / 2.0);
    s=1./(1+exp(-initial_v(:,(k-1)*p+1:p*k)));
    temp=s-rd3;
    temp(temp>=0)=1;
    temp(temp<0)=-1;
    initial_x{k}=temp;
...
%         for k=1:pop
%         initial_x{k}=initial_xx(:,(k-1)*p+1:p*k);
%         end
...
% obj=sum(diag(0.5*x'*d*x))+0.5*beta*sum((sum(x,2)-2+p).*(sum(x,2)-2+p))+0.5*beta*sum((sum(x,1)-2*n/p+n).*(sum(x,1)-2*n/p+n));
% flag=0.5*beta*sum((sum(x,2)-2+p).*(sum(x,2)-2+p))+0.5*beta*sum((sum(x,1)-2*n/p+n).*(sum(x,1)-2*n/p+n));
...
for i=1:m2
    ...
    x(:,i)=DiscreteHopfield(y,n,d,p,u0,alpha,beta);
    %x(:,i)=DiscreteHopfieldSyn(x(:,i),hat_W,theta_(:,i),iter,sizepop);
end
function [x]=DiscreteHopfield(x,n,d,p,u0,alpha,beta)
A=((1));
B=((alpha)); 
D=((beta));
s1=((sum(x,2)));
